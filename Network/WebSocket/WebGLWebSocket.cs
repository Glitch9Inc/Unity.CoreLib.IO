...
namespace Glitch9.IO.Network.WebSocket
{
    // this is the client implementation used by browsers
    public class WebGLWebSocket : IWebSocket
    {
        public string SubProtocol { get; }
        public WebSocketState State => (WebSocketState)WebSocketGetState(instanceId);
        ...
        public UniTask ConnectAsync(Uri uri, CancellationToken token, params RESTHeader[] headers)
        {
            return ConnectAsync(uri.ToString(), headers, token);
        }

        public UniTask ConnectAsync(string uri, CancellationToken token, params RESTHeader[] headers)
        {
