...
namespace Glitch9.IO.Files
{
    ...
    public static class WavUtils
    {
        ...
        public static AudioClip ToAudioClip(byte[] fileBytes, int offsetSamples = 0, string name = "wav")
        {
            ...
            Debug.AssertFormat(audioFormat == 1 || audioFormat == 65534, "Detected format code '{0}' {1}, but only PCM and WaveFormatExtendable uncompressed formats are currently supported.", audioFormat, formatCode);
        ...
        public static byte[] FromAudioClip(AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = "recordings")
        {
            ...
            const ushort BIT_DEPTH = 16; //BitDepth (audioClip);
            ...
            int fileSize = audioClip.samples * BLOCK_SIZE_16_BIT + HEADER_SIZE; // BlockSize (bitDepth)

            // chunk descriptor (riff)
            WriteFileHeader(ref stream, fileSize);
            // file header (fmt)
            WriteFileFormat(ref stream, audioClip.channels, audioClip.frequency, BIT_DEPTH);
            // data chunks (data)
            WriteFileData(ref stream, audioClip, BIT_DEPTH);
            ...
            if (saveAsFile)
            {
                string pathFormat = "{0}/{1}/{2}.{3}";
                filepath = string.Format(pathFormat, Application.persistentDataPath, dirname, DateTime.UtcNow.ToString("yyMMdd-HHmmss-fff"), "wav");
        ...
        private static int BlockSize(ushort bitDepth)
        {
            ...
            {
                32 => sizeof(int), // 32-bit -> 4 bytes (Int32)
                16 => sizeof(short), // 16-bit -> 2 bytes (Int16)
                8 => sizeof(sbyte), // 8-bit -> 1 byte (sbyte)
                _ => throw new Exception(bitDepth + " bit depth is not supported."),
            };
        }
        ...
        private static string FormatCode(ushort code)
        {
            switch (code)
            {
                ...
                case 3: return "IEEE";
                case 7: return "μ-law";
                case 65534: return "WaveFormatExtensable";
            }
        ...
        private static int WriteFileData(ref MemoryStream stream, AudioClip audioClip, UInt16 bitDepth)
        {
            ...
            int subchunk2Size = Convert.ToInt32(audioClip.samples * BLOCK_SIZE_16_BIT); // BlockSize (bitDepth)
