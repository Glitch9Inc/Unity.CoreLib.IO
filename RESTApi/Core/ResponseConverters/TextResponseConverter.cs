...
namespace Glitch9.IO.RESTApi
{
    internal static class TextResponseConverter
    {
        private static TRes Deserialize<TRes>(string jsonString, RESTClient client)
        {
            ...
            {
                client.InternalLogger.ResponseError("Deserialize failed. JSON settings are null.");
                return default;
            }

            if (string.IsNullOrEmpty(jsonString))
            {
                client.InternalLogger.ResponseError("JSON string is null or empty.");
                return default;
            }
            ...
            {
                client.InternalLogger.ResponseError($"Failed to deserialize <color=yellow>{typeName}</color> from JSON: {e.Message}");
                return default;
            }
        }
